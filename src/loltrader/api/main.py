    ...
    with connect() as c:
        rows = c.execute(
            ...
            WHERE m.status IN ('active', 'open')
              AND {ticker_filter}
              -- Show all active sub-markets, even with no price data. This
              -- keeps both teams visible per map so the user can see the
              -- complete match structure. Empty markets render with '—'
